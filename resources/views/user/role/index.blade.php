                            ...
                            <table class="table table-striped table-2">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td> {{ $item->name }}</td>
                                        <td>
                                            @foreach ($item->getPermissionNames() as $p)
                                            <p class="badge badge-success">{{ $p }}</p>
                                            @endforeach
                                        </td>
                                        @can('role-list', 'role-delete', 'role-edit')
                                        <td class="text-center">
                                            @can('role-edit')
                                            <a href="{{ route('role.edit', $item->id) }}" class="icon-edit"
                                                data-toggle="tooltip" data-placement="top" title=""
                                                data-original-title="Edit Role"><i
                                                    class="fa-regular fa-pen-to-square"></i></a>
                                            @endcan
                                            @can('role-list')
                                            <a href="{{ route('role.show', $item->id) }}" class="icon-info px-3"
                                                data-toggle="tooltip" data-placement="top" title=""
                                                data-original-title="Detail Info"><i class="fa-regular fa-eye"></i></a>
                                            @endcan
                                            @can('role-delete')
...
<script>
    $(document).ready(function() {
            $('.icon-delete').click(function() {
                ...
                var idRole = $(this).attr('data-id-delete');
                var actionDel = "{{ route('role.destroy', ':id') }}".replace(':id', idRole);
