        ...
        <div class="col-12 col-md-12 col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('laporan.update', [$data->nip_pegawai, $data->id_lapor]) }}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="id_usul"
                                        value="@if (!empty($data->id_usul)) {{ $data->id_usul }} @endif">
                                    <input type="hidden" name="id_pengiriman"
                                        value="@if (!empty($data->id_lapor)) {{ $data->id_lapor }} @endif">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="search" name="nama_diklat" maxlength="250"
                                        value="{{ strip_tags($data->nama_diklat) }}" placeholder="..Nama Diklat.."
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="custom-file mb-3 custom-input-pdf">
                                        <input type="file" class="custom-file-input @error('file_surat_laporan')
                                                is-invalid @enderror" name="file_surat_laporan"
                                            id="customFileSuratLaporan"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label style="display: block;">File Sertifikat (STTPP) <span
                                            class="text-danger">*</span></label>
                                    <div class="custom-icon-pdf2">
                                        <a class="style-icon-pdf2" href="#"
                                            onclick="togglePdfReader(event, '{{ asset('Lamp_Sertifikat') }}/{{ $data->file_sttpp }}')">
                                            <i class="fa-regular fa-file-pdf"></i>
                                        </a>
                                    </div>
                                    <div class="custom-file mb-3 custom-input-pdf2">
                                        <input type="file"
                                            class="custom-file-input @error('file_sttpp') is-invalid @enderror"
                                            name="file_sttpp" id="customFileSertifikat"
                                            value="{{ asset('lampiran') }}/{{ $data->file_sttpp }}"
                                            accept="application/pdf">
                                        <label class="custom-file-label" for="customFileSertifikat">{{ $data->file_sttpp
                                            }}</label>
                                        @error('file_sttpp')
                                        <div class="invalid-feedback mt-3">
                                            {{ $message }}
                            ...
                            <div class="col-12">
                                <div id="pdfReaderContainer" style="display: none;">
                                </div>
                            </div>
                            ...
                            <div class="col-md-6" id="alasan">
                                <div class="form-group">
                                    ...
                                    <textarea name="alasan" class="form-control" style="height: 100px"
                                        placeholder="..Isikan alasan apabila laporan ditolak/perlu perbaikan..">{{ strip_tags($data->alasan) }}</textarea>
                                </div>
                            </div>
                            @endcan

                            @cannot('laporan-approve')
                            @if($data->status == "2")
                            <input class="form-check-input" type="hidden" name="status" value="3" id="status6" checked>
                            <div class="col-md-6" id="#"></div>
                            <div class="col-md-6" id="alasan">
                                <div class="form-group">
                                    <label id="text-catatan"></label>
                                    <textarea name="alasan" class="form-control" style="height: 100px"
                                        placeholder="..Isikan alasan apabila laporan ditolak/perlu perbaikan.."
                                        readonly>{{ strip_tags($data->alasan) }}</textarea>
                                </div>
                            </div>
                            @endif
                            @endcannot
...
<script>
    $(document).ready(function() {
            $('#sectionViewPdf').append(
                '<object data="{{ asset('Lamp_Laporan') }}/{{ $data->file_surat_laporan }}" type="application/pdf" width="100%" height="600px">' +
...
<script>
    $(document).ready(function() {
            $('#sectionViewPdfSertifikat').append(
                '<object data="{{ asset('Lamp_Sertifikat') }}/{{ $data->file_sttpp }}" type="application/pdf" width="100%" height="600px">' +
                '<p>Maaf, File PDF anda tidak bisa ditampilkan.</p></object>'
            );
    ...
    $(document).ready(function() {
        function toggleAlasan() {
            if ($('#status4').is(':checked')) {
                $('#alasan').show();
                $('#text-catatan').html("Catatan Jika Ditolak <span class='text-danger'>*</span>");
            } else if ($('#status2').is(':checked') || $('#status5').is(':checked')) {
                $('#alasan').show();
                // $('#status1').attr('disabled', true);
                if ($('#status2').is(':checked')) {
                    $('#text-catatan').html("Catatan <span class='text-danger'>*</span>");
                } else {
                    $('#text-catatan').html("Catatan Perbaikan <span class='text-danger'>*</span>");
                }
            } else {
                $('#alasan').hide();
            }
        ...
        // Toggle alasan on page load
        toggleAlasan();

        // Toggle alasan on status change
...
<script>
    // if $status == 2 maka disable semua radio buton kecuali radio button 2
    $(document).ready(function() {
        if ("{{ $data->status }}" == 2) {
            // alasan show and readonly
            $('#alasan').show();
            $('#text-catatan').html("Catatan Perbaikan <span class='text-danger'>*</span>");
        }
    });
...
<script>
    function togglePdfReader(event, pdfUrl) {
        event.preventDefault(); // Prevent the default behavior (e.g., scrolling to the top)

        var pdfReaderContainer = document.getElementById('pdfReaderContainer');

        if (pdfReaderContainer.style.display === 'none') {
            // If PDF reader is hidden, show it and load the PDF
            pdfReaderContainer.innerHTML = '<iframe src="' + pdfUrl + '" width="100%" height="600px"></iframe>';
            pdfReaderContainer.style.display = 'block';
        } else {
            // If PDF reader is visible, hide it
            pdfReaderContainer.innerHTML = '';
