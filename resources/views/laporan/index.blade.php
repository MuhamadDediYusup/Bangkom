...
<script>
    var start = 0;
            ...
            function fetchDataLaporanDitinjau() {
                $.ajax({
                    ...
                    console.log(data);
                    var dataTable = $('#lap_ditinjau').DataTable({
                        destroy: true,
                        ordering: false,
                        stateSave: true,
                        autoWidth: false,
                        ...
                        "columns": [{
                            ...
                            {
                                "render": function(data, type, row) {
                                    return row.jenis_diklat + "<br>" + row.sub_jenis_diklat;
                                }
                            },
                            {
                                "data": "nama_diklat"
                            ...
                            },
                            {
                                "render": function(data, type, row) {
                                    return row.status == '3' ?
                                        "<span class='badge badge-warning badge-outlined' data-toggle='tooltip' data-placement='top' title='" +
                                        moment(row.entry_time).format('DD-MM-YYYY') +
                                        "'> Diperbaiki</span>" :
                                        "<span class='badge badge-warning' data-toggle='tooltip' data-placement='top' title='" +
                                        moment(row.entry_time).format('DD-MM-YYYY') +
                                        "'> Ditinjau</span>";
                                }
                            },
                            {
                                "render": function(data, type, row) {
                                    return "@can('laporan-edit', 'laporan-delete') <td class='text-center'> @can('laporan-edit') <a href = '{{ url('/laporan/edit/') }}/" +
                                        row.nip + "/" + row.id_lapor + "/" + row.status +
                                        "' data-toggle = 'tooltip' data-placement = 'top' title = 'Edit Laporan'> <i class = 'fa-regular fa-pen-to-square'> </i></a> @endcan @can('laporan-delete') <a href='javascript:void(0)' class='icon-delete' onclick='confirmDelete("+row.id_lapor+")' data-id-delete='" +
                                        row.id_lapor +
                                        "' data-toggle = 'tooltip' data-placement = 'top' title = 'Hapus Laporan'> <i class = 'fa-solid fa-trash-can text-danger'> </i></a> @endcan <div> @endcan";
                                }
                            }
                        ],
                        columnDefs: [{
            ...
            $('#ditinjau_next').click(function() {
                end += 50;
                fetchDataLaporanDitinjau();
            });

            function fetchDataLaporanDiperbaiki() {
                $.ajax({
                    ...
                    var dataTable = $('#lap_diperbaiki').DataTable({
                        destroy: true,
                        ordering: false,
                        stateSave: true,
                        autoWidth: false,
            ...
            $('#diperbaiki_next').click(function() {
                end += 50;
                fetchDataLaporanDiperbaiki();
            });

            function fetchDataLaporanDisetujui() {
                $.ajax({
                    ...
                    var dataTable = $('#lap_disetujui').DataTable({
                        destroy: true,
                        ordering: false,
                        stateSave: true,
                        autoWidth: false,
            ...
            $('#disetujui_next').click(function() {
                ...
                fetchDataLaporanDisetujui();
            });

            function fetchDataLaporanDitolak() {
                $.ajax({
                    ...
                    var dataTable = $('#lap_ditolak').DataTable({
                        destroy: true,
                        ordering: false,
                        stateSave: true,
                        autoWidth: false,
...
<script src="https://unpkg.com/sweetalert2@7.18.0/dist/sweetalert2.all.js"></script>
<script>
    function confirmDelete(idLaporan) {
        $(document).ready(function() {
            $('#deletemodal').modal({
                backdrop: 'static',
                keyboard: false,
            ...
            var actionDel = "{{ url('laporan/destroy') }}" + "/" + idLaporan;
